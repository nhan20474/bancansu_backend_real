...
router.get('/all', requireAuth, lopController.getAllLopWithDetails);

// Đảm bảo route /count được đặt TRƯỚC các route động như /:id
router.get('/count', lopController.countLop);

// Đảm bảo route tìm kiếm nằm TRƯỚC route động /:id
router.get('/search', lopController.searchLop);

router.get('/', requireAuth, lopController.getAllLop);
